*
*   This program computes stopping cross sections using the Extended Wave 
*   Packet Model (UWPM) proposed by Archubi & Arista (2017)
*
      program EWPM
      ...
      common/plas/enl,chi2,qbar,gamma
      COMMON/CK/XK                    ! test
...
c.... Default velocity grid in a.u.
     ...
     |            28.28D0, 47.00D0,  63.24D0/
      ...
      gamma = 1.d0
c**********      READ DATA      ***********************
      open(10,file='ewpm.inp',status='old')
      read(10,*) vmin
      read(10,*) vmax
      ...
      qbar = q1 * number_electrons**(1.d0/3.d0)
      chi2 = 1.d0 / (pi*qbar)
      wp_p2 = gamma * qbar**3 / sqrt(pi)
      atomic_density_au = wp_p2 / (4.d0*pi*number_electrons)
...
c.... f-sum rules -  Archubi & Arista (2017), Eq. (10)
      XK  = 10.0
      W1  = 1.E-5
      W2  = 500.d0
      TOL = 1.E-6
      CALL GABQ(fun2,W1,W2,SUM1,TOL,IER)
      theovalue = pi / 2.d0 * wp_p2

      WRITE(*,1000) int(number_electrons),q1,enl,atomic_density_au
      write(*,1005) chi2,qbar
      write(*,1001) theovalue,SUM1

c**********      OUTPUT FILE     ***************************
      WRITE(*,*)'Enter the output file name'
      READ(*,*) filename

      str = trim(filename)//'_EWPM.dat'
      open(7,file=str,status='unknown')
      WRITE(7,1002) trim(filename)
      write(7,1000) number_electrons,q1,enl,atomic_density_au
      write(7,1005) chi2,qbar
      WRITE(7,1003)
      CLOSE(7)
      
1001  format(/"# Checking f-sum rule:"/,
     |       "# Theo. value = ",f20.6,2x,"  Num. value = ",f12.6/)
1002  format('# PROTON STOPPING POWER OF H on BOUND ELECTRONS',1x,
     |       'USING THE EWPM'/,
     |       '# Target:',1x,a)
1000  format('# Number of electrons: ',i4/,
     |       "# Kaneko's screening parameter: ",f12.3/,
     |       "# Electron binding energy: ",f12.3,1x,"a.u."/,
     |       '# Atomic density:',1x,1pe12.3,1x,'a.u.')
1005  format("# Chi^2: ",f12.4/,
     |       "# q_bar: ",f12.4)
1003  FORMAT(/'# V(au)',3X,'SP(au/atom)',4X,'SP(10^-15eVcm2/atom)')
...
c.... define integration limits for k variable
        EIK1 = 1.D-5
c        EIK2 = 4.d0*(v+xkf)
        if (v.lt.1) epsmax = 75.d0
        if (v.ge.1) epsmax = 25.d0
        EIK2 = epsmax*v
...
*********************************************************
      SUBROUTINE elfuwpm(w,xk,elf)
       IMPLICIT REAL*8 (A-H,O-Z)
       COMPLEX*16 df
       common/plas/enl,chi2,xkf,gamma
       ...
       if (w.ge.enl) then
        ...
        ukw = wg / (xk*xkf)
        zkw = xk / (2.d0*xkf)
        e1 = gamma*chi2*(G(ukw+zkw) - G(ukw-zkw)) / (8.d0*zkw**3.d0)
        e1 = 1.d0 + e1 
        e2 = gamma * pi * chi2 / (8.d0*zkw**3.d0)
        e2 = e2 * (exp(-(ukw - zkw)**2.d0) - exp(-(ukw + zkw)**2.d0))
        df = dcmplx(e1, e2)
        elf = dimag(-1.0d0 / df)
c        elf = e2
       else
